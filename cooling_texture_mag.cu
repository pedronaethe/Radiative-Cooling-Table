...
void CreateTexture()
{
    ...
    cudaMalloc3DArray(&cuCoolArray, &channelDesc, make_cudaExtent(nx,ny,nz), 0);
...
__global__ void cooling_function(cudaTextureObject_t my_tex, float a1, float a2, float a3)
{
...
int main()
{
    ...
    while (loop > 1)
    {
	    ...
	    printf("Te value:\n");
	    scanf("%f", &read3);
	    cooling_function<<<1, 1>>>(coolTexObj, read1, read2, read3);
        sleep(1);
