...
double f(double x, double edens, double etemp, double mag_field)
{
    if (thetae(etemp) < 0.03)
    {
        return pow(C_euler, 1.8899 * pow(x, 1. / 3.)) -
               2.49 * pow(10., -10.) * 12. * C_pi * edens * scale_height(etemp) / (mag_field) * 1 / (2 * pow(thetae(etemp), 5.)) *
                   (1 / pow(x, 7. / 6.) + 0.4 / pow(x, 17. / 12.) + 0.5316 / pow(x, 5. / 3.));
    }
    else
    {
        return pow(C_euler, 1.8899 * pow(x, 1. / 3.)) - 2.49 * pow(10., -10.) * 12. * C_pi * edens * scale_height(etemp) / (mag_field) * 1 /
...
double secant(double f(double x, double edens, double etemp, double mag_field))
{
    int iter = 1;
    double x1 = 1.;
    double x2 = 20000.;
    ...
    do
    {
        x3 = (x1 * f(x2, edens, etemp, mag_field) - x2 * f(x1, edens, etemp, mag_field)) / (f(x2, edens, etemp, mag_field) - f(x1, edens, etemp, mag_field));
        ...
        // printf("x3 = %lf\n", x3);
        // printf("x1 = %lf\n", x1);
    } while (fabs(f(x3, edens, etemp, mag_field)) > eps && iter <= maxSteps);
    return x1;
}
...
double rsync(double edens, double etemp, double mag_field)
{
    ...
    {
        double result =
            2 * C_pi * BOLTZ_CGS * etemp * pow(crit_freq(edens, etemp, mag_field), 3.) / (3 * scale_height(etemp) * pow(C_CGS, 2.)) +
            6.76 * pow(10., -28.) * edens / (bessk2(1 / thetae(etemp)) * pow(a1, 1. / 6.)) *
                (1 / pow(a4, 11. / 2.) * gammq(11. / 2., a4 * pow(crit_freq(edens, etemp, mag_field), 1. / 3.)) +
                 a2 / pow(a4, 19. / 4.) * gammq(19. / 4., a4 * pow(crit_freq(edens, etemp, mag_field), 1. / 3.)) + a3 / pow(a4, 4.) * (pow(a4, 3.) * crit_freq(edens, etemp, mag_field) + 3 * pow(a4, 2.) * pow(crit_freq(edens, etemp, mag_field), 2. / 3.) + 6 * a4 * pow(crit_freq(edens, etemp, mag_field), 1. / 3.) + 6) * pow(C_euler, -a4 * pow(crit_freq(edens, etemp, mag_field), 1. / 3.)));
        return result;
    }
    else
    {
...
double soptical_depth(double edens, double etemp)
{
    double result = 2. * edens * THOMSON_CGS * scale_height(etemp);
    return result;
}
...
double absoptical_depth(double edens, double etemp, double mag_field)
{
    double result = 1. / (4. * C_sigma * pow(etemp, 4.)) * scale_height(etemp) * totalthincooling_rate(edens, etemp, mag_field);
    return result;
}
...
int main()
{
    ...
    //	printf("O valor do tau_total é:%le\n", total_optical_depth(radius, edens, etemp));
    //  printf("o valor do cooling total é:%le\n", total_cooling(radius, edens, etemp, mag_field));
    // printf("\ndobro: %lf\n", dobro(f));
    FILE *file_radius;
    ...
    fprintf(file_result, "mag_field, e_density, temperature, cooling\n");
    // run for logarithm values
    while (fscanf(file_mag_field, "%lf,", &mag_field) == 1)
    {
        ...
        while (fscanf(file_e_density, "%lf,", &edens) == 1)
        {
            ...
            {
                    fprintf(file_result, "%.2f, %.2f, %.2f,", mag_field, edens, etemp);
                    edens = pow(10., edens);
                    etemp = pow(10., etemp);
                    mag_field = pow(10., mag_field);
                    cooling = log10(total_cooling(edens, etemp, mag_field));
                    edens = log10(edens);
                    etemp = log10(etemp);
                    mag_field = log10(mag_field);
