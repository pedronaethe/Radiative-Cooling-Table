...
double bessk1(double xbess) {
    ...
    if (xbess <= 2.0) {
        ...
        ans = (exp(-xbess) / sqrt(xbess)) * (1.25331414 + y * (0.23498619
                                                               + y *
                                                                 (-0.3655620e-1 + y * (0.1504268e-1 + y * (-0.780353e-2
                                                                                                           + y *
                                                                                                             (0.325614e-2 +
                                                                                                              y *
                                                                                                              (-0.68245e-3)))))));
...
void gcf(double *gammcf, double agam, double xgam,
         double *gln) { //Function used in the calculation of incomplete gamma function
    int i;
    ...
    if (thetae(Te) > 0.03) {
        double result =
                ...
                (1 / pow(a4, 11. / 2.) * gammq(11. / 2., a4 * pow(crit_freq(ne, Te), 1. / 3.)) +
                 a2 / pow(a4, 19. / 4.) * gammq(19. / 4., a4 * pow(crit_freq(ne, Te), 1. / 3.)) + a3 / pow(a4, 4.) *
                                                                                                  (pow(a4, 3.) *
                                                                                                   crit_freq(ne, Te) +
                                                                                                   3 * pow(a4, 2.) *
                                                                                                   pow(crit_freq(ne,
...
double comptonization_factor_artur(double ne, double Te) {
    ...
    double result = 1 + prob * (A - 1) / (1 - prob * A) * (1 - pow((PLANCK_CGS * crit_freq(ne, Te) /
                                                                    (3 * thetae(Te) * ERM_CGS * pow(C_CGS, 2.))),
                                                                   -1 - log(prob) / log(A)));
    return result;
}
    ...
    while(fscanf(file_radius, "%lf,", &R) == 1) {
        ...
        while(fscanf(file_e_density, "%lf,", &ne) == 1) {
            ...
            while(fscanf(file_temperature, "%lf,", &Te) == 1) {
                ...
                fprintf(file_result,"%le, %le, %le, %le\n", R, ne, Te, cooling);
            }
        }
    }
