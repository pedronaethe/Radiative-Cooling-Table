...
/******************************************************/
/* CONSTANTS ADDED TO COOLING FUNCTIONS               */
/******************************************************/


#define eps 3e-7			/*used in secant method to iteration*/
#define maxSteps 1e3			/*used in secant method to iteration*/
...
#define Rs 2. * G_CGS * C_Mbh / (pow(C_CGS, 2.))

/*Mass of the Black hole*/
#define C_Mbh  10. * C_Msun
#define C_GM  C_Mbh * G_CGS
...
#define kappa (GAMMA-1.)/(GAMMA) *pow(rho_0, (1.- GAMMA)) * (CONST_2 + C_GM/(r_0-Rs) - C_GM/2. * r_0/(pow(r_0-Rs, 2.)))
...
/******************************************************************************************************/
/*         FUNCTIONS USED TO COOL DOWN THE DISK AS DESCRIBED BY Esin 1996, 1996ApJ...465..312E        */
/*	   AND DESCRIBED ALSO BY NARAYAN YI 1995ApJ...452..710N                                       */
/* 	   FUNCTIONS INTRODUCED BY PEDRO NAETHE MOTTA                                                 */
/******************************************************************************************************/

/*********************AUXILIARY MATHEMATICAL SPECIAL FUNCTIONS CALCULATION*****************************/
...
{ //Calculation of lower incomplete gamma function
	int n = 0;
	if (agam < 0){
		agam = - agam;
		if (n < 1){
...
{ //Calculation of upper incomplete gamma function
	double gamser,gammcf,gln;
	if (xgam < (agam+1.0)) { 
		gser(&gamser,agam,xgam,&gln);
		return 1.0-gamser; 
...
__device__ double Te (double * ph, double r, double gam){
	double tg = (gam - 1) * ph[UU]/ph[RHO];
	return tg/(pow(r/r_0, -1) + 2.);
}

__device__ double ne (double * ph){
	return ph[RHO]/ERM_CGS;
}

__device__ double Bmag (struct of_state *  q){
	double bsq = dot(q->bcon,q->bcov);
	return pow(bsq, 1./2.); 
}

__device__ double thetae (double * ph, double r, double gam){ //Calculation of Temperature parametrization
	double result = (BOLTZ_CGS) * Te(ph, r, gam)/((ERM_CGS) * (pow(C_CGS, 2.)));
	return result;
}

__device__ double sound_speed(double * ph){ //Calculation of the soundspeed
	double result = pow((GAMMA) * (kappa) * pow(ne(ph) * 1.14 * C_amu, (GAMMA-1.)), 1./2.);
	return result;
}

__device__ double scale_height(double r, double * ph){
	double result = pow(r/C_GM, 1./2.)*sound_speed(ph)*(r-Rs);
	return r;
}

__device__ double f(double x, double r, double * ph, double gam, struct of_state * q){ //transcedental equation to be solved
	if (thetae(ph, r, gam) < 0.03){
		return pow(M_E, 1.8899 * pow(x, 1./3.)) - 2.49* pow(10., -10.) * 12. * M_PI * ne(ph) * r/(Bmag(q)) * 1/(2*pow(thetae(ph, r, gam), 5.)) * (1/pow(x, 7./6.) + 0.4/pow(x, 17./12.) + 0.5316/pow(x, 5./3.));
	} else{
		return pow(M_E, 1.8899 * pow(x, 1./3.)) - 2.49* pow(10., -10.) * 12. * M_PI * ne(ph) * r/(Bmag(q)) * 1/(pow(thetae(ph, r, gam), 3.) * bessk2(1/thetae(ph, r, gam))) * (1/pow(x, 7./6.) + 0.4/pow(x, 17./12.) + 0.5316/pow(x, 5./3.));
}

}

__device__ double secant(double f(double x, double r, double * ph, double gam, struct of_state * q), double * ph, double r, double gam, struct of_state * q){
    int iter=1;
    double x1 = 1.;
    double x2 = 20000.;
    double x3;
    do{
        x3=(x1*f(x2, r, ph, gam, q)-x2*f(x1, r, ph, gam, q))/(f(x2, r, ph, gam, q)-f(x1, r, ph, gam, q));
        x1=x2;
        x2=x3;
        iter++;
	//printf("x3 = %lf\n", x3);
	//printf("x1 = %lf\n", x1);
    }while(fabs(f(x3, r, ph, gam, q))>eps&&iter<=maxSteps);
	return x1;
}

__device__ double bremmstrahlung_ee (double * ph, double r, double gam){
	double result;
	if (thetae(ph, r, gam) < 1) {
		result = 2.56 * pow(10., -22.) * pow(ne(ph), 2.) * pow(thetae(ph, r, gam), 3./2.) * (1 + 1.10 * thetae(ph, r, gam) + pow(thetae(ph, r, gam), 2.) - 1.25 * pow(thetae(ph, r, gam), 5./2.));
		
	} else {
		result = 3.40 * pow(10., -22.) * pow(ne(ph), 2.) * thetae(ph, r, gam) * (log(1.123 * thetae(ph, r, gam)) + 1.28);
	}
	return result;
}

__device__ double bremmstrahlung_ei (double * ph, double r, double gam){
	double result;
	if (thetae(ph, r, gam) > 1) {
		result = 1.48 * pow(10., -22.) * pow(ne(ph), 2.) * (9*thetae(ph, r, gam))/(2 * M_PI) * (log(1.123*thetae(ph, r, gam) + 0.48) + 1.5);
		
	} else {
		result = 1.48 * pow(10., -22.) * pow(ne(ph), 2.) * 4 * pow((2* thetae(ph, r, gam)/pow(M_PI,3.)), 1./2.) * (1 + 1.781 * pow(thetae(ph, r, gam), 1.34));
	}
	return result;
}

__device__ double bremmscooling_rate (double * ph, double r, double gam){
	double result = bremmstrahlung_ee(ph, r, gam) + bremmstrahlung_ei(ph, r, gam);
	return result;
}

__device__ double crit_freq (double * ph, double r, double gam, struct of_state * q){
	return (1.5) * (2.80e6 * Bmag(q)) * pow(thetae(ph, r, gam), 2.) * secant(f, ph, r, gam, q);
}

//Synchtron radiation calculation
__device__ double rsync (double * ph, double r, double gam, struct of_state * q){
	double a1 = 2/(3 * (2.8e6 * Bmag(q)) * pow(thetae(ph, r, gam), 2.));
	double a2 = 0.4/pow(a1, 1./4.);
	double a3 = 0.5316/pow(a1, 1./2.) ;
	double a4 = 1.8899 *pow(a1, 1./3.);
	if (thetae(ph, r, gam) > 0.03) {
		return 2 * M_PI * BOLTZ_CGS * Te(ph, r, gam) * pow(crit_freq(ph, r, gam, q), 3.)/(3 * scale_height(r, ph) * pow(C_CGS, 2.)) + 6.76 * pow(10., -28. ) * ne(ph)/(bessk2(1/thetae(ph, r, gam)) * pow(a1, 1./6.)) * (1/pow(a4, 11./2.) * gammq(11./2., a4 * pow(crit_freq(ph, r, gam, q), 1./3.)) + a2/pow(a4, 19./4.) * gammq(19./4., a4 * pow(crit_freq(ph, r, gam, q), 1./3.)) + a3/pow(a4, 4.) * (pow(a4, 3.) * crit_freq(ph, r, gam, q) + 3 * pow(a4, 2.) * pow(crit_freq(ph, r, gam, q), 2./3.) + 6 * a4 * pow(crit_freq(ph, r, gam, q), 1./3.) + 6) * pow(M_E, -a4 * pow(crit_freq(ph, r, gam, q), 1./3.)));
}	else{	
		return 2 * M_PI * BOLTZ_CGS * Te(ph, r, gam) * pow(crit_freq(ph, r, gam, q), 3.)/(3 * scale_height(r, ph) * pow(C_CGS, 2.)) + 6.76 * pow(10., -28. ) * ne(ph)/(2*pow(thetae(ph, r, gam), 2.) * pow(a1, 1./6.)) * (1/pow(a4, 11./2.) * gammq(11./2., a4 * pow(crit_freq(ph, r, gam, q), 1./3.)) + a2/pow(a4, 19./4.) * gammq(19./4., a4 * pow(crit_freq(ph, r, gam, q), 1./3.)) + a3/pow(a4, 4.) * (pow(a4, 3.) * crit_freq(ph, r, gam, q) + 3 * pow(a4, 2.) * pow(crit_freq(ph, r, gam, q), 2./3.) + 6 * a4 * pow(crit_freq(ph, r, gam, q), 1./3.) + 6) * pow(M_E, -a4 * pow(crit_freq(ph, r, gam, q), 1./3.)));

}
}

__device__ double comptonization_factor (double * ph, double r, double gam, struct of_state * q){
	double thompson_opticaldepth = 2 * ne(ph) * THOMSON_CGS * Te(ph, r, gam);
	double Afactor = 1 + 4 * thetae(ph, r, gam) + 16 * pow(thetae(ph, r, gam), 2.);
	double maxfactor = 3 * BOLTZ_CGS * Te(ph, r, gam)/(PLANCK_CGS * crit_freq(ph, r, gam, q));
	double jm = log(maxfactor)/log(Afactor);
	double s = thompson_opticaldepth + pow(thompson_opticaldepth, 2.);
	double result = pow(M_E, s*(Afactor -1))*(1 - gammp(jm - 1, Afactor * s)) + maxfactor * gammp(jm +1, s);
	if (isnan(result)){
	return maxfactor * gammp(jm +1, s);
}
	
	return pow(M_E, s*(Afactor -1))*(1 - gammp(jm - 1, Afactor * s)) + maxfactor * gammp(jm +1, s);
...
__device__ double soptical_depth(double * ph, double r){
	double result = 2. * ne(ph) * THOMSON_CGS * scale_height(r, ph);
	return result;
}
__device__ double comptonization_factor_artur(double * ph, double r, double gam, struct of_state * q) {
	double prob = 1 - pow(M_E, - soptical_depth(ph, r));
	double A = 1 + 4 * thetae(ph, r, gam) + 16 * pow(thetae(ph, r, gam), 2.);
	double result = 1 + prob*(A-1)/(1 - prob*A) * (1 - pow((PLANCK_CGS * crit_freq(ph, r, gam ,q)/(3 * thetae(ph, r, gam)* ERM_CGS * pow(C_CGS, 2.))), -1 - log(prob)/log(A)));
	return result;
	
}

__device__ double totalthincooling_rate (double * ph, double r, double gam, struct of_state * q){
	return bremmstrahlung_ee(ph, r, gam) + bremmstrahlung_ei(ph, r, gam) + rsync(ph, r, gam, q) * comptonization_factor(ph, r, gam, q);
}
 // Absorption optical depth
__device__ double absoptical_depth(double * ph, double r, double gam, struct of_state * q){
	double result = 1./(4. *STEFAN_CGS * pow(Te(ph, r, gam), 4.)) * scale_height (r, ph) *  totalthincooling_rate(ph, r, gam, q);
	return result;
}

// Total optical depth
__device__ double total_optical_depth(double * ph, double r, double gam, struct of_state * q){
	double result = soptical_depth(ph, r) + absoptical_depth(ph, r, gam, q);
	return result;
}
// Total cooling with thin and thick disk
__device__ double total_cooling (double * ph, double r, double gam, struct of_state * q){
	double result = 4. * STEFAN_CGS * pow(Te(ph, r, gam), 4.)/scale_height(r, ph) * 1/(3 * total_optical_depth(ph, r, gam, q)/2. * pow(3., 1./2.) + 1./absoptical_depth(ph, r, gam, q));
	return result;
}
__device__ void misc_source(double *  ph, int icurr, int jcurr, struct of_geom *  geom, struct of_state *  q, double *  dU,
	...
	double bsq = dot(q->bcon,q->bcov);
	/*printf("O raio é: %le\n", r);
	printf("A temperatura é: %le\n", Te(ph, r, gam));
	printf("O campo magnético é: %le\n", Bmag(q));
	printf("A densidade eletronica é: %le\n", ne(ph));
	printf("One of the roots is: %le\n",secant(f(x, r, ph, gam, q),));
	...
	if (bsq / ph[RHO]<1. || r<10.){
		if (fabs(q->ucov[0] * lambda)*Dt<0.1*fabs(int_energy)){
			dU[UU] += -q->ucov[0] * (lambda + total_cooling(ph, r, gam, q));
